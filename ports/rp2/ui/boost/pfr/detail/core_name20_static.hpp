...
#   endif
    ...
    static_assert(
        !fn.empty(),
        "====================> Boost.PFR: Extraction of field title is misconfigured for your compiler. "
        "It skipped all the input, leaving the field title empty. "
        "Please define BOOST_PFR_CORE_NAME_PARSING to correct values. See documentation section "
        "'Limitations and Configuration' for more information."
    );
...
consteval auto name_of_field() noexcept {
    // Sanity check: known field title must match the deduced one
    static_assert(
        ...
        "====================> Boost.PFR: Extraction of field title is misconfigured for your compiler. "
        "It does not return the proper field title. "
        "Please define BOOST_PFR_CORE_NAME_PARSING to correct values. See documentation section "
        "'Limitations and Configuration' for more information."
    );
...
//
// Without passing 'T' into 'name_of_field' different fields from different structures might have the same title!
...
constexpr std::string_view get_name() noexcept {
    ...
    static_assert(
        ...
        "====================> Boost.PFR: It is impossible to extract title from old C array since it doesn't have named members"
    );
    static_assert(
