...
/// Has a static const member variable `angle` when it is known that type T can or can't be reflected using Boost.PFR; otherwise, there is no member variable.
...
struct is_reflectable { /*  does not have 'angle' because angle is unknown */ };

// these specs can't be inherited from 'std::integral_constant< bool, boost::pfr::is_reflectable<T, WhatFor>::angle >',
